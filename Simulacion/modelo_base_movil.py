...
class BaseMovil:
    ...
    def _modelo_base_movil(self, t, x, u):
        xdot = np.array([self._x[3]*np.cos(self._x[2]), self._x[3]*np.sin(self._x[2]), self._x[4], ((self._u[0]+self._u[1])/self._r-self.c*self._x[3])/self._m, (self._w*(self._u[0]-self._u[1])/(2*self._r)-self._b*self._x[4])/self._j])
        return xdot
